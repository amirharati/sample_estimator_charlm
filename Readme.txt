...
Update:
I added RnnLmS2S.py  that uses seq2seq model for LM modeling. Basically no encoder and simple decoder and does not have the isses mentioned for the RNN model with estomator API.
However, it seems to me it is converging much slower. For example, for a charecter LM the simple RNN model can generate okish sequences even after 500
 iterations, but  RNN using seq2seq model has even probelm generating <EOS> symble after few 1000 iteration and tends to generate reptative patterns.
